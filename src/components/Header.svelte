...
<header class="flex justify-between items-center">
    <h1 class="hidden lg:flex font-bold text-xl underline-offset-4 text-indigo-600">{fragment.charAt(0).toUpperCase()+fragment.substring(1)}</h1>

    <button on:click={toggleMenu} class="fixed lg:hidden w-screen min-h-screen bottom-0 bg-black opacity-40 z-10 {isVisible?'block ':'hidden'}"></button>

    <nav id="nav" class="slideIn lg:flex p-2 rounded-lg lg:-mt-3 {isVisible ? 'rounded-none lg:rounded-lg lg:bg-slate-700 bg-slate-950  flex flex-col h-screen lg:h-fit lg:relative w-4/5 lg:w-fit lg:p-2  z-50 fixed bottom-0 p-5 pt-10 left-0' : 'hidden lg:bg-slate-700'}">
